...
void si_output_scanner_command(string outdir_name)
{
    ...
    outfile << _SI_DFT_INS_IF << endl;
    outfile << _SI_DFT_INS_GW << endl;
...
int si_network_scan(struct RegionTreeNode **regn_forest, int tree_num, int &budget, ofstream &addr_total_res)
{
    ...
    time_t fn_now = time(0); 

    cout << "[Filtering] Starting APD filtering." << endl;
    system("grepcidr -v -f ipv6-bl-merged.txt targets.txt > targets.txt.bl");
    cout << "[Filtering] APD filtering ended, starting BL filtering." << endl;
    system("grepcidr -v -f aliased-prefixes.txt targets.txt.bl > targets_clean.txt");
    cout << "[Filtering] Saving intermediate files." << endl;

    std::stringstream cmd1;
    std::stringstream cmd2;
    cmd1 << "cp targets_clean.txt targets_" << fn_now << "_" << iteration << "_clean.txt";
    cmd2 << "cp targets.txt targets_" << fn_now << "_" << iteration << ".txt";
    system(cmd1.str().c_str());
    system(cmd2.str().c_str());
    iteration++;

    // 2. Call the scanner ZMapv6 to search active addresses.
    cout << "[Filtering] Finished copying, starting scanning..." << endl;
    system("zmap --probe-module=icmp6_echoscan --ipv6-target-file=targets_clean.txt --output-file=result.txt --ipv6-source-ip=2001:4ca0:108:42::28 --rate=4000 --cooldown-time=10 --interface=eno3 --gateway-mac=00:1c:73:00:00:99");
